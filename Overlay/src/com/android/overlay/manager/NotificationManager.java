...
public class NotificationManager implements OnInitializedListener,
	 ...
	 */
	protected final List<NotificationProvider<? extends NotificationItem>> providers;
	...
	protected NotificationManager() {
		this.application = RunningEnvironment.getInstance();
	...
	public Intent createClearNotificationsIntent(Context context) {
		Intent intent = null;
		try {
			intent = new Intent(context, Class.forName(context
					.getString(Environment
							.getResId("R.string.notification_clear"))));
	...
	public Intent createReconnectionActivityIntent(Context context) {
		...
		try {
			intent = new Intent(context, Class.forName(context
					.getString(Environment
							.getResId("R.string.page_reconnection"))));
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		}
		return intent;
	...
	protected void runPersistentNotification() {
		String contentText = "+++";
		Intent persistentIntent = createReconnectionActivityIntent(application
				.getApplicationContext());

		persistentNotification.icon = Environment
				.getResId("R.drawable.ic_stat_normal");
		persistentNotification
				.setLatestEventInfo(application.getApplicationContext(),
						application.getString(Environment
								.getResId("R.string.application_name")),
						contentText, PendingIntent.getActivity(
								application.getApplicationContext(), 0,
								persistentIntent,
								PendingIntent.FLAG_UPDATE_CURRENT));
